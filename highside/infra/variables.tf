...
variable "azure_government_environment" {
  description = "Azure Government cloud environment (usgovernment, usgovernmentsecret, usgovernmenttopsecret)"
  type        = string
  default     = "usgovernment"

  validation {
    condition     = contains(["usgovernment", "usgovernmentsecret", "usgovernmenttopsecret"], var.azure_government_environment)
    error_message = "Must be usgovernment (IL4/IL5), usgovernmentsecret (IL6), or usgovernmenttopsecret (TS/SCI)."
  }
}

variable "location" {
  description = "Azure Government region (e.g., usgovvirginia, usgovarizona)"
  type        = string
...
variable "classification" {
  description = "Data classification (CUI, Secret, TopSecret)"
  type        = string
  default     = "CUI"

  validation {
    condition     = contains(["CUI", "Secret", "TopSecret"], var.classification)
    error_message = "Classification must be CUI, Secret, or TopSecret."
  }
}
...
variable "aptly_vm_size" {
  description = "VM size for Aptly serving instance"
  type        = string
...
variable "internal_dns_suffix" {
  description = "Internal DNS suffix for repo access (e.g., apt.internal.local)"
  type        = string
  default     = "apt.internal.local"
}
...
variable "enable_internal_load_balancer" {
  description = "Enable internal load balancer for high availability"
  type        = bool
  default     = false
}

variable "allowed_client_subnets" {
  description = "CIDR blocks of client subnets allowed to access Aptly repo"
  type        = list(string)
  default     = []
}
